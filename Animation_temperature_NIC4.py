...
def PRINT_PNG(w,time,grid,prec,scale):
	...
	file_name = str(calendar.month_name[time+1]) +"_%s.png" %(grid);  fig.savefig("Temperature/%s" %(file_name), dpi=200); ann.remove()
...
def COLORBAR(var_min,var_max):
	...
	cb.ax.set_title('[$^\circ$C]',fontsize=12)
...
if __name__ == '__main__':
	...
	var_min,var_max = 4,20 	# minimum and maximum variable magnitude
	...
	for i in range(365*3):
		...
		temp[lo(t).month-1] += n1.variables['temp'][i,-1,:,:]
