...
def remap(tt,  src_file, src_varname, wts, src_grd, dst_grd, src_grd_file,order,grid):
	...
	time=cdf.variables['time'][0:tt]
	if src_varname=='sst':
		nc1 = Dataset(src_grd_file, 'r', format='NETCDF4'); lat1=nc1.variables['lat'][:]; lon1=nc1.variables['lon'][:]; nc1.close()
	else:
		nc1 = Dataset(src_grd_file, 'r', format='NETCDF4'); lon1 = nc1.variables['lon'][44:74]; lat1 = nc1.variables['lat'][31:62]; nc1.close()
	...
	if src_varname=='sss':
		dst_varname = 'salt'; long_name = 'surface surface salinity'; units = 'Psu'; field = 'sss, scalar, series'
	if src_varname=='ssu':
	...
	for i in range(len(time)):
		...
		dst_cont = pyroms.remapping.remap(src_varz, wts_file, spval=spval) #print np.shape(dst_cont)
