...
def func(name):
	...
	for i in range(3):
		...
		plt.subplot(gs1[0, i]).plot(pts[:,0], pts[:,1])
		plt.annotate('ROMS Results: %s' %(vliz[i]), xy=(0, 0), xytext=(-1, 1))
		plt.xlim([-1.25,1.25])
		plt.ylim([-1.25,1.25])
		ptsz = get_ellipse_coords(a=mj[i], b=mn[i], x=0, y=0, angle=-ic[i], k=2)
		plt.subplot(gs1[1, i]).plot(ptsz[:,0], ptsz[:,1])
		plt.annotate('VLIZ Data: %s' %(vliz[i]), xy=(0, 0), xytext=(-1, 1))
		plt.xlim([-1.25,1.25])
		plt.ylim([-1.25,1.25])
		#plt.axis('equal')

	fig.savefig('Ellipse_plot_M2_%s' %(name), dpi=200)
