...
import matplotlib as mpl; import os, sys; import calendar;  from ezhen.plotbcz import *;  from ezhen.Get_grid import Get_grid_ODYSSEA_Nest; import cmocean

lo = lambda x: datetime(2006,1,1,0,0,0) + timedelta(seconds=x)
folder = os.path.abspath("Animation/Currents/Month")
cmap=cmocean.cm.speed; mpl.cm.rainbow
...
if PARAM == 'AV':
	ncdata1 = Dataset('Replotting/%s_bulk_AVG.nc' %(name), 'r', format='NETCDF4')
elif PARAM == 'INST':
	ncdata1 = Dataset('Replotting/OWc_bulk.nc', 'r', format='NETCDF4')
t=ncdata1.variables['time'][:]; sp=np.shape(ncdata1.variables['sst'][0])
lons_p, lats_p = ncdata1.variables['lon_rho'][:],ncdata1.variables['lat_rho'][:]
mask=ncdata1.variables['mask_rho'][:]
...
def PRINT_PNG(u_m,v_m,rs_w,w,time):
	...
	if PARAM=='AV':
		kk = m1.quiver(x1[::prec,::prec],y1[::prec,::prec],u_m[::prec,::prec],v_m[::prec,::prec],angles='xy')#,linewidth=rs_w[::prec,::prec].flatten()*10,angles='xy')
	...
	file_name = os.path.abspath(folder + "/%s_%s_%s" %(name,time+1,monthname)+".png");  fig.savefig(file_name, dpi=80); ann.remove(); kk.remove() #; CS3.remove();
...
if PARAM=='AV':
	...
	for i in range(len(t)):
		...
		roms_u[lo(int(t[i])).month-1] = roms_u[lo(int(t[i])).month-1]+ncdata1.variables['ssu'][i,:,:]
		roms_v[lo(int(t[i])).month-1] = roms_v[lo(int(t[i])).month-1]+ncdata1.variables['ssv'][i,:,:]
...
for n in range(a,b):
	if PARAM=='AV':
		...
		u10 = np.flipud(ru)/day[n]
		v10 = np.flipud(rv)/day[n]
		ww = roms[n]/day[n]
	elif PARAM=='INST':
	...
	#u_map_unmsk /= mag_scale*0.1#0.1						# and makes them suitable
	#v_map_unmsk /= mag_scale*0.1#0.1	
	ww = np.ma.masked_where(abs(mask-1),ww)
	...
	u_map,v_map,roms_w,w = u_map_unmsk, v_map_unmsk, roms_w_unmsk, ww
	PRINT_PNG(u_map,v_map,np.flipud(roms_w),w,n)
