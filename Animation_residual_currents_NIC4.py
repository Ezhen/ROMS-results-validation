...
def PRINT_PNG(u,v,w,time,grid,prec,scale):
	...
	file_name = str(calendar.month_name[time+1]) +"_%s.png" %(grid);  fig.savefig("Residuals/%s" %(file_name), dpi=200); ann.remove(); kk.remove()
...
if __name__ == '__main__':
	...
	var_min,var_max = 0,0.5 	# minimum and maximum variable magnitude
	...
	for i in range(365*3):
		...
		ru[lo(t).month-1] += n1.variables['u_eastward'][i,-1,:,:]
		rv[lo(t).month-1] += n1.variables['v_northward'][i,-1,:,:]
	...
	for n in range(12):
		...
		vv = rv[n]/day[n]
		ww = (uu*uu+vv*vv)**0.5
		u = np.ma.masked_where(abs(mask-1),uu)
		v = np.ma.masked_where(abs(mask-1),vv)
		w = np.ma.masked_where(abs(mask-1),ww)
		PRINT_PNG(u,v,w,n,grid,prec,scale)
