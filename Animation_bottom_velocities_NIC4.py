...
def PRINT_PNG(u,v,w,time,grid,prec,scale):
	...
	file_name = str(d2(time).strftime("%d_%B_%Y_%H")) +"_%s.png" %(grid);  fig.savefig("Bottom/%s" %(file_name), dpi=200); ann.remove(); kk.remove()
...
if __name__ == '__main__':
	...
	#path = '/scratch/ulg/mast/eivanov/NESTING10RESULTS/NSS_HIS_20062008_parent.nc'; grid = 'OW'; prec = 2; scale = 25
	path = '/scratch/ulg/mast/eivanov/NESTING10RESULTS/NSS_TW_HIS_20062008_parent.nc'; grid = 'TW'; prec = 2; scale = 25
	...
	var_min,var_max = -1,1 	# minimum and maximum variable magnitude
	...
	for n in range(t1,t2):
		u10,v10 = n1.variables['u_eastward'][n,0,:,:],n1.variables['v_northward'][n,0,:,:]
